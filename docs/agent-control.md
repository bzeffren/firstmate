...
When `interrupt` or a busy `exit` delivers that key and the agent remains alive, the control plane separately corrects the interrupted ledger record to `idle`/`fm-interrupt` through `fm_busy_record_manual_interrupt` in `bin/fm-busy-lib.sh`.
The correction is bound to the busy generation and record sequence captured before key delivery, so a lifecycle event from a newer turn wins instead of being overwritten as idle.
`bin/fm-send.sh`'s `--key Escape` path calls the same executable helper rather than keeping a second correction path.
